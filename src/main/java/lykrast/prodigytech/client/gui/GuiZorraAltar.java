...
public class GuiZorraAltar extends GuiContainer {
    ...
    private static final ResourceLocation ENCHANTMENT_TABLE_GUI_TEXTURE =
            ProdigyTech.resource("textures/gui/zorra_altar.png");
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        this.fontRenderer.drawString(
                new TextComponentTranslation("container." + ProdigyTech.MODID + ".zorra_altar").getUnformattedText(),
                12,
                5,
                4210752);
        this.fontRenderer.drawString(
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        for (int row = 0; row < 3; ++row) {
            ...
            if (cost == 0) drawTexturedModalRect(i1, midY + 14 + 19 * row, 0, 185, 108, 19);
                ...
                if (((leaves < leafCost || mc.player.experienceLevel < cost) && !mc.player.capabilities.isCreativeMode)
                        || container.enchantId[row]
                                == -1) // Forge: render buttons as disabled when enchantable but enchantability not
                // met on lower levels
                {
                    // Disabled background
                    ...
                    fontrenderer.drawSplitString(
                            glyph, j1, midY + 16 + 19 * row, glyphLength, (shadowColor & 16711422) >> 1);
                ...
                fontrenderer.drawStringWithShadow(
                        displayCost,
                        (float) (j1 + 86 - fontrenderer.getStringWidth(displayCost)),
                        (float) (midY + 16 + 19 * row + 7),
                        shadowColor);
            }
        }
    }
                    ...
                    if (enchantment != null) {
                        ...
                        if (j == 2)
                            list.add(""
                                    ...
                                    + I18n.format("container.prodigytech.zorra_altar.unknown"));
                    ...
                    if (!flag) {
                        ...
                        if (this.mc.player.experienceLevel < cost) {
                            list.add(TextFormatting.RED
                                    + I18n.format(
                                            "container.enchant.level.requirement", this.container.enchantCost[j]));
                        } else {
                            String s;
                            ...
                            TextFormatting textformatting =
                                    leaves >= leafCost ? TextFormatting.GRAY : TextFormatting.RED;
