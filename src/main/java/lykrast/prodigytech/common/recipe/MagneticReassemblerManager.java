...
public class MagneticReassemblerManager extends SimpleRecipeManager {
    ...
    public void init() {
        ...
        if (RecipeUtil.itemExists("exnihilocreatio:block_endstone_crushed"))
            addRecipe(
                    new ItemStack(RecipeUtil.getExteriorItem("exnihilocreatio:block_endstone_crushed")),
                    new ItemStack(Blocks.END_STONE));
        if (RecipeUtil.oreExists("crushedGranite"))
            addRecipe("crushedGranite", new ItemStack(Blocks.STONE, 1, BlockStone.EnumType.GRANITE.getMetadata()));
        if (RecipeUtil.oreExists("crushedDiorite"))
            addRecipe("crushedDiorite", new ItemStack(Blocks.STONE, 1, BlockStone.EnumType.DIORITE.getMetadata()));
        if (RecipeUtil.oreExists("crushedAndesite"))
            addRecipe("crushedAndesite", new ItemStack(Blocks.STONE, 1, BlockStone.EnumType.ANDESITE.getMetadata()));
        ...
        addRecipe(
                "dustTinyFerramic", new ItemStack(ModItems.ferramicNugget), Config.magneticReassemblerProcessTime / 9);
        addRecipe("dustEnergion", new ItemStack(ModItems.energionCrystalSeed));

        addRecipe(
                new ItemStack(ModItems.infernoFuel),
                new ItemStack(ModItems.infernoCrystal),
                Config.magneticReassemblerProcessTime * 2);
