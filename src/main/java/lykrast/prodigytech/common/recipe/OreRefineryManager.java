...
public class OreRefineryManager extends SimpleRecipeManagerSecondaryOutput {
    ...
    public SimpleRecipeSecondaryOutput addRecipe(
            ItemStack in, ItemStack out, ItemStack secondary, float secondaryChance) {
        return addRecipe(
                new SimpleRecipeSecondaryOutput(in, out, secondary, Config.oreRefineryProcessTime, secondaryChance));
    }

    public SimpleRecipeSecondaryOutput addRecipe(
            String inOre, ItemStack out, ItemStack secondary, float secondaryChance) {
        return addRecipe(
                new SimpleRecipeSecondaryOutput(inOre, out, secondary, Config.oreRefineryProcessTime, secondaryChance));
    }
    ...
    public void init() {
        ...
        // Use Thermal Expansion secondary dusts first, then IndustrialCraft 2, then Immersive
        // Engineering
        ...
        ItemStack sulfur =
                RecipeUtil.oreExists("dustSulfur") ? RecipeUtil.getPreferredOreStack("dustSulfur") : ItemStack.EMPTY;

        addRecipe(
                "oreCoal",
                new ItemStack(ModItems.coalDust, Config.oreRefineryOreMultiplier),
                sulfur,
                Config.oreRefineryChance);
        if (!Config.autoOreRecipes)
            addRecipe(
                    "oreIron",
                    new ItemStack(ModItems.ironDust, Config.oreRefineryOreMultiplier),
                    new ItemStack(ModItems.goldDust),
                    Config.oreRefineryChance);
        if (!Config.autoOreRecipes)
            addRecipe("oreGold", new ItemStack(ModItems.goldDust, Config.oreRefineryOreMultiplier));
        addRecipe(
                "oreLapis",
                new ItemStack(Items.DYE, 6 * Config.oreRefineryOreMultiplier, 4),
                sulfur,
                Config.oreRefineryChance);
        ...
        addRecipe(
                "oreQuartz",
                new ItemStack(ModItems.quartzDust, Config.oreRefineryOreMultiplier),
                sulfur,
                Config.oreRefineryChance);
    }
