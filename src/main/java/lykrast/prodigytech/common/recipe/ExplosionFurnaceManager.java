...
public class ExplosionFurnaceManager {
    ...
    public static ExplosionFurnaceRecipe addRecipe(
            ItemStack input, ItemStack output, int power, ItemStack reagent, int craftPerReagent) {
        return addRecipe(new ExplosionFurnaceRecipe(input, output, power, reagent, craftPerReagent));
    }

    // Oredict input, normal reagent
    public static ExplosionFurnaceRecipe addRecipe(
            String input, int inputCount, ItemStack output, int power, ItemStack reagent, int craftPerReagent) {
    ...
    public static ExplosionFurnaceRecipe addRecipe(
            ItemStack input, ItemStack output, int power, String reagent, int craftPerReagent) {
        return addRecipe(new ExplosionFurnaceRecipe(input, output, power, reagent, craftPerReagent));
    }

    // Oredict input, oredict reagent
    public static ExplosionFurnaceRecipe addRecipe(
            String input, int inputCount, ItemStack output, int power, String reagent, int craftPerReagent) {
        return addRecipe(new ExplosionFurnaceRecipe(input, inputCount, output, power, reagent, craftPerReagent));
    }
    ...
    public static boolean isValidInput(ItemStack check) {
        for (ExplosionFurnaceRecipe recipe : RECIPES) if (recipe.isValidInput(check)) return true;

        return false;
    }

    public static boolean isValidReagent(ItemStack check) {
        for (ExplosionFurnaceRecipe recipe : RECIPES) if (recipe.isValidReagent(check)) return true;
    ...
    public static void init() {
        ...
        // Stone
        // Those were initially found by blowing up TNT in a 11x11x11 block and counting how many blocks
        // were destroyed
    ...
    public static class ExplosionFurnaceRecipe {
        ...
        private ExplosionFurnaceRecipe(
                Ingredient input,
                int inputCount,
                ItemStack output,
                int power,
