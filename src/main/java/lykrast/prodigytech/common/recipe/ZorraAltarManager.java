...
public class ZorraAltarManager {
    public static final ZorraAltarManager SWORD = new ZorraAltarManager(), BOW = new ZorraAltarManager();
    ...
    public static void init() {
        ...
        if (Loader.isModLoaded("enderio")) {
            ...
            if (!hasSoulbound
                    && soulbound != null
                    && (soulbound.canApply(checkerSword) || soulbound.canApply(checkerBow))) {
                hasSoulbound = true;
        ...
        if (Loader.isModLoaded("cofhcore")) {
            // CoFH Core makes enchants unapplicable and useless instead of not registering them when
        ...
        if (Loader.isModLoaded("apotheosis")) {
            ...
            if (BOW.addModdedEnchantBonusLevel("apotheosis:true_infinity", 0, checkerBow))
                BOW.removeEnchant(Enchantments.INFINITY);
        }
    }
     ...
     * @param maxLvl maximum applicable level
     * @param checker ItemStack that should be enchantable with the target enchantment, used to check
     *     if the enchantment is disabled
    ...
    public boolean addModdedEnchant(String key, int maxLvl, ItemStack checker) {
        ...
        // CoFH Core makes enchantments unapplicable and useless instead of not registering them when
        // disabled individually
        // So we use this dummy ItemStack to check if it's applicable
