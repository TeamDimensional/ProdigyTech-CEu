    ...
    public static class HotAirInfoProvider implements IProbeInfoProvider {
        ...
        public void addProbeInfo(
            ...
            if (te instanceof TileWormholeFunnel) {
                ...
                probeInfo.text(IProbeInfo.STARTLOC
                        + (funnel.isLinked() ? WORMHOLE_LINKED : WORMHOLE_UNLINKED)
                        + IProbeInfo.ENDLOC);
                ...
                if (funnel.isInput()) {
                    ...
                    if (funnel.getOutAirTemperature() > 0)
                        probeInfo.element(
                                new TextFormatInt(TooltipUtil.TEMPERATURE_INPUT, funnel.getOutAirTemperature()));
                } else {
                    if (funnel.getOutAirTemperature() > 0)
                        probeInfo.element(
                                new TextFormatInt(TooltipUtil.TEMPERATURE_OUT, funnel.getOutAirTemperature()));
                }
                ...
                if (hotair instanceof HotAirMachine) {
                    ...
                    if (casted.getInAirTemperature() > 0)
                        probeInfo.element(
                                new TextFormatInt(TooltipUtil.TEMPERATURE_INPUT, casted.getInAirTemperature()));
                }
                if (hotair.getOutAirTemperature() > 0)
                    probeInfo.element(new TextFormatInt(TooltipUtil.TEMPERATURE_OUT, hotair.getOutAirTemperature()));
            }
        }
    }
        ...
        public void addProbeInfo(
            ...
            if (te instanceof TileAeroheaterTartaric) {
                ...
                if (tartaric.isBurningSomething()) {
                    probeInfo.progress(
                            tartaric.getBurnLeft(),
                            tartaric.getBurnMax(),
                            probeInfo
                                    .defaultProgressStyle()
                                    .height(6)
                                    .showText(false)
                                    .filledColor(0xFFE19900)
                                    .alternateFilledColor(0xFFE19900));
                    probeInfo.progress(
                            tartaric.getStokerLeft(),
                            tartaric.getStokerMax(),
                            probeInfo
                                    .defaultProgressStyle()
                                    .height(6)
                                    .showText(false)
                                    .filledColor(0xFF78ACC8)
                                    .alternateFilledColor(0xFF78ACC8));
                }
