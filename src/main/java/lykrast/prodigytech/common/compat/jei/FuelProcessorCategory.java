...
public class FuelProcessorCategory extends ProdigyCategory<SimpleRecipeWrapper> {
    ...
    public static void registerRecipes(IModRegistry registry) {
        ...
        fuels.stream().forEach(fuel -> {
            if (TileFuelProcessor.isValidInput(fuel)) list.add(wrap(fuel, guiHelper));
        });
    ...
    private static SimpleRecipeWrapper wrap(ItemStack fuel, IGuiHelper guiHelper) {
        return new SimpleRecipeWrapper(
                fuel.copy(),
                new ItemStack(TileFuelProcessor.getResultingPellet(fuel), TileFuelProcessor.getPelletsAmount(fuel)),
                TileFuelProcessor.getProcessTime(fuel),
                guiHelper);
    }
}
