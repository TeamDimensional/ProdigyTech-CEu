...
public class OreRefinery {
    ...
    private static SimpleRecipeSecondaryOutput recipe(IItemStack in, IItemStack out, int time) {
        return new SimpleRecipeSecondaryOutput(
                CraftTweakerHelper.toItemStack(in), CraftTweakerHelper.toItemStack(out), time);
    }

    private static SimpleRecipeSecondaryOutput recipe(
            IItemStack in, IItemStack out, IItemStack sec, float chance, int time) {
        return new SimpleRecipeSecondaryOutput(
                CraftTweakerHelper.toItemStack(in),
                CraftTweakerHelper.toItemStack(out),
                CraftTweakerHelper.toItemStack(sec),
                time,
                chance);
    }
    ...
    private static SimpleRecipeSecondaryOutput recipe(
            IOreDictEntry in, IItemStack out, IItemStack sec, float chance, int time) {
        return new SimpleRecipeSecondaryOutput(
                in.getName(), CraftTweakerHelper.toItemStack(out), CraftTweakerHelper.toItemStack(sec), time, chance);
    }
    ...
    public static void addRecipe(
            IItemStack in, IItemStack out, IItemStack sec, @Optional float chance, @Optional int time) {
        if (in == null) throw new IllegalArgumentException("Input cannot be null");
        if (out == null) throw new IllegalArgumentException("Output cannot be null");
    ...
    @ZenMethod
    public static void addRecipe(
            IOreDictEntry in, IItemStack out, IItemStack sec, @Optional float chance, @Optional int time) {
        if (in == null) throw new IllegalArgumentException("Input cannot be null");
        if (out == null) throw new IllegalArgumentException("Output cannot be null");
        if (chance <= 0) chance = Config.oreRefineryChance;
        if (chance > 1) chance = 1;
    ...
    private static class Add implements IAction {
        ...
        public String describe() {
            String message =
                    "Adding Ore Refinery recipe for " + recipe.getOutput().getDisplayName();
            if (recipe.hasSecondaryOutput())
                message += " + " + recipe.getSecondaryOutput().getDisplayName();
