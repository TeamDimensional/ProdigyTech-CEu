...
public class ExplosionFurnace {
    ...
    private static ExplosionFurnaceRecipe recipe(
            IItemStack in, IItemStack out, int ep, IItemStack reagent, int craftPerReagent) {
        return new ExplosionFurnaceRecipe(
                CraftTweakerHelper.toItemStack(in),
                CraftTweakerHelper.toItemStack(out),
                ep,
                CraftTweakerHelper.toItemStack(reagent),
                craftPerReagent);
    }

    // Oredict input, normal reagent
    private static ExplosionFurnaceRecipe recipe(
            IOreDictEntry in, IItemStack out, int ep, IItemStack reagent, int craftPerReagent) {
        return new ExplosionFurnaceRecipe(
                in.getName(),
                in.getAmount(),
                CraftTweakerHelper.toItemStack(out),
    ...
    private static ExplosionFurnaceRecipe recipe(
            IItemStack in, IItemStack out, int ep, IOreDictEntry reagent, int craftPerReagent) {
        return new ExplosionFurnaceRecipe(
                CraftTweakerHelper.toItemStack(in),
                CraftTweakerHelper.toItemStack(out),
                ep,
    ...
    private static ExplosionFurnaceRecipe recipe(
            IOreDictEntry in, IItemStack out, int ep, IOreDictEntry reagent, int craftPerReagent) {
        return new ExplosionFurnaceRecipe(
                in.getName(),
                in.getAmount(),
                CraftTweakerHelper.toItemStack(out),
    ...
    public static void addRecipe(IIngredient in, IItemStack out, int ep) {
        ...
        if (in instanceof IItemStack) CraftTweakerAPI.apply(new Add(recipe((IItemStack) in, out, ep)));
        else if (in instanceof IOreDictEntry) CraftTweakerAPI.apply(new Add(recipe((IOreDictEntry) in, out, ep)));
