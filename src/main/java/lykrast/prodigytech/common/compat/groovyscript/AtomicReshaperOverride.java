...
public class AtomicReshaperOverride extends AtomicReshaper {
    ...
    public AtomicReshaper.RecipeBuilder recipeBuilder() {
        throw new NotImplementedException("recipeBuilder() is currently not supported for Atomic Reshaper, use builder() instead");
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<AtomicReshaperManager.AtomicReshaperRecipe> {
        ...
        public void validate(GroovyLog.Msg msg) {
            ...
            msg.add(data != null && !data.machines.contains("atomic_reshaper"), "infusion {} cannot be used with Atomic Reshaper");
        }
        ...
        public @Nullable AtomicReshaperManager.AtomicReshaperRecipe register() {
            ...
            if (inputItem instanceof OreDictIngredient) {
                ...
                recipe = new AtomicReshaperManager.AtomicReshaperRecipe(oredict, time, new InfusionCost(infusion, primordium), getRecipeOutput());
                ModSupport.PRODIGY_TECH.get().atomicReshaper.add(recipe);
            } else {
                for (ItemStack it : inputItem.getMatchingStacks()) {
                    recipe = new AtomicReshaperManager.AtomicReshaperRecipe(it, time, new InfusionCost(infusion, primordium), getRecipeOutput());
                    ModSupport.PRODIGY_TECH.get().atomicReshaper.add(recipe);
                }
            }
