...
public class BlockZorraLeaves extends BlockLeaves implements ICustomStateMapper, ICustomItemBlock {
    ...
    protected static final AxisAlignedBB COLLISION_AABB =
            new AxisAlignedBB(0.0625D, 0.0625D, 0.0625D, 0.9375D, 0.9375D, 0.9375D);

    public BlockZorraLeaves(float hardness, float resistance) {
        setHardness(hardness);
        setResistance(resistance);
        setDefaultState(getDefaultState()
                .withProperty(SOAKED, false)
        ...
        if (!worldIn.isRemote
            ...
            if (valid > 0) {
                worldIn.setBlockState(
                        validPos[rand.nextInt(valid)], getDefaultState().withProperty(CHECK_DECAY, Boolean.FALSE));
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState()
                .withProperty(SOAKED, Boolean.valueOf((meta & 2) > 0))
    ...
    public void setCustomStateMapper() {
        ModelLoader.setCustomStateMapper(
                this, (new StateMap.Builder()).ignore(CHECK_DECAY, DECAYABLE).build());
    }
