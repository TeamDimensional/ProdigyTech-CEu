...
public class BlockEnergionCrystal extends BlockGeneric implements ICustomStateMapper, ICustomItemBlock {
    ...
    public BlockEnergionCrystal(float hardness, float resistance, int harvestLevel) {
        ...
        setDefaultState(
                blockState.getBaseState().withProperty(AGE, Integer.valueOf(0)).withProperty(DEFUSED, false));
        setLightLevel(0.6F);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (age < 5) {
            if (net.minecraftforge.common.ForgeHooks.onCropsGrowPre(
                    worldIn, pos, state, rand.nextInt((int) (25 / Config.energionGrowthSpeed) + 1) == 0)) {
    ...
    public void setCustomStateMapper() {
        ModelLoader.setCustomStateMapper(
                this, (new StateMap.Builder()).ignore(DEFUSED).build());
    }
}
