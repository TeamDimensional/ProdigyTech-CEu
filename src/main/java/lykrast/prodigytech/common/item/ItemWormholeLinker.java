...
public class ItemWormholeLinker extends Item {
        ...
        if (stack.hasTagCompound() && stack.getTagCompound().hasKey("Pos")) {
            ...
            if (!tile.isInRange(targetPos)) {
                player.sendStatusMessage(
                        new TextComponentTranslation("status.prodigytech.wormhole_linker.error.distance"), true);
            ...
            if (targetTile == null) {
                player.sendStatusMessage(
                        new TextComponentTranslation("status.prodigytech.wormhole_linker.error.lost"), true);
                return EnumActionResult.SUCCESS;
            }
            ...
            if (tile.isInput() == targetTile.isInput()) {
                player.sendStatusMessage(
                        new TextComponentTranslation("status.prodigytech.wormhole_linker.error.incompatible"), true);
                return EnumActionResult.SUCCESS;
            }
            ...
            if (tile.createLink(targetTile)) {
                player.sendStatusMessage(
                        new TextComponentTranslation("status.prodigytech.wormhole_linker.success"), true);
                if (player instanceof EntityPlayerMP)
                    ProdigyTech.networkChannel.sendTo(
                            new PacketWormholeDisplay(targetPos, pos), (EntityPlayerMP) player);
            } else
                player.sendStatusMessage(
                        new TextComponentTranslation("status.prodigytech.wormhole_linker.error"), true);
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        if (stack.hasTagCompound() && stack.getTagCompound().hasKey("Pos"))
            tooltip.add(I18n.format(stack.getTranslationKey() + ".tooltip.linking"));
