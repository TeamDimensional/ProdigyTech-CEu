...
public class TileExplosionFurnace extends TileMachineInventory {
        ...
        if (!exp.isEmpty()) {
            ...
            if (explosive != null) {
                ...
                for (int slot = 2; slot <= 4; slot++) {
                    ...
                    if (!stack.isEmpty()) {
                        // Used to check if we should try for another recipe with the same output if we run out
                        // of reagent
                                ...
                                while (!shouldCraft && stack.getCount() >= inCount && power >= cost) {
                                    ...
                                    if (requireReagent) {
                                        ...
                                        if (craftLeft <= 0) {
                                            ...
                                            // findRecipe can only give something where reagent matches or isn't
                                            // required
                                            if (!reagent.isEmpty() && recipe.isValidReagent(reagent)) {
                                                ...
                                                // No reagent left, for another craft, don't consume anything and get
                                                // ready
                                                // for another crafting loop
                                                shouldCraft = true;
                                                break;
                                            }
                                        }
    ...
    private ProdigyInventoryHandler invHandler = new ProdigyInventoryHandler(
            this, 9, 0, new boolean[] {true, true, true, true, true, true, false, false, false}, new boolean[] {
    ...
    public <T> T getCapability(Capability<T> capability, EnumFacing facing) {
        if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) return (T) invHandler;
