...
public class TileFuelProcessor extends TileHotAirMachineSimple {
    ...
    public static boolean isValidInput(ItemStack stack) {
        // Item must be fuel, burn for at least 1 item, not burn for more than 64 items, not have a
        // container (no buckets) and must not be a Fuel Pellet
        int burn = TileEntityFurnace.getItemBurnTime(stack);
        return burn >= 200
                && burn <= CAP
                && stack.getItem().getContainerItem(stack).isEmpty()
                && !BLACKLIST.contains(stack.getItem());
    }
    ...
    private void smelt() {
        ...
        // Assume that whatever was in the output slot is the correct output and that there was enough
        // room
