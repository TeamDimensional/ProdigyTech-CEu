...
public class TileFoodEnricher extends TileHotAirMachineSimple {
    ...
    public static boolean isValidInput(ItemStack stack) {
        ...
        return !(item.getHealAmount(stack) >= Config.foodEnricherFoodCap
                && item.getSaturationModifier(stack) >= Config.foodEnricherSaturationCap);
    }

    public static int getProcessTime(ItemStack stack) {
        // Assumes it is a valid input
        // Base formula is ((improved food restored + improved saturation restored) - (food restored +
        // saturation restored))^2
        // Simplified with wolfram alpha to (2*(food * saturation ratio increase + saturation ratio *
        // food increase + food increase * saturation ratio increase) + food increase)^2
    ...
    protected boolean canProcess() {
        ...
        updateCachedResult();
        // Assume that if something was inserted it is valid input and that whatever is in the output is
        // Purified Food
        ItemStack curOutput = getStackInSlot(1);
        return curOutput.isEmpty()
                || (ItemStack.areItemStackTagsEqual(curOutput, cachedResult)
                        && curOutput.getCount() + 1 <= ModItems.purifiedFood.getItemStackLimit());
    }
    ...
    private void smelt() {
        ...
        // Assume that whatever was in the output slot is matching and there's room, since canProcess
        // checks for that
